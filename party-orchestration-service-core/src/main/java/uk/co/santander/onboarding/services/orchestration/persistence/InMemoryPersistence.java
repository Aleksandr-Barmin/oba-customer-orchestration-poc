...
public class InMemoryPersistence implements StateMachinePersist<OrchestrationState, OrchestrationEvent, UUID>,
        StateMachineRepository<OrchestrationState, OrchestrationEvent, UUID> {

    private final Map<UUID, StateMachineContext<OrchestrationState, OrchestrationEvent>> storage = new ConcurrentHashMap<>();
    ...
    @Override
    public void write(StateMachineContext<OrchestrationState, OrchestrationEvent> context, UUID contextObj) throws Exception {
        storage.put(contextObj, context);
    }

    @Override
    public StateMachineContext<OrchestrationState, OrchestrationEvent> read(UUID contextObj) throws Exception {
