...
public class StateMachineService {
    ...
    @Autowired
    private StateMachinePersister<OrchestrationState, OrchestrationEvent, UUID> persister;
    ...
    public OrchestrationState sendEvent(UUID applicationId, OrchestrationEvent event) {
        return withMachine(applicationId, (machine) -> {
    ...
    private OrchestrationState withMachine(UUID applicationId, ExecutionCallback callback) {
        final StateMachine<OrchestrationState, OrchestrationEvent> stateMachine = getStateMachine(applicationId);
    ...
    private StateMachine<OrchestrationState, OrchestrationEvent> getStateMachine(UUID applicationId) {
        final StateMachine<OrchestrationState, OrchestrationEvent> stateMachine = stateMachineFactory.create();
